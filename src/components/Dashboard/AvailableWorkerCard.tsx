...
export default function AvailableWorkerCard({ worker }: { worker: Worker }) {
  return (
    <div className="bg-[#FFFFFF0D] border border-[#FFFFFF33] p-6 rounded-2xl space-y-4">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3
              ...
              className={`font-semibold text-[#FFFFFF] ${FONTSIZE[18]}`}
            >
              {worker.name}
            </h3>
            <p
              style={FONTWEIGHT[400]}
              className={`text-sm text-[#99A1AF] ${FONTSIZE[12]}`}
            >
              {worker.role}
            </p>
          </div>
        ...
        <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium bg-gray-500/20 text-gray-300 border border-gray-500/40">
          <div
            style={FONTWEIGHT[400]}
            className={`w-4 h-4 rounded-full flex items-center justify-center ${FONTSIZE[12]}`}
          >
            <img
              src={white}
              alt="Available"
